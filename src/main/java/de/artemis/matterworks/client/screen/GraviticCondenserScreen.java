...
public class GraviticCondenserScreen extends net.minecraft.client.gui.screens.inventory.AbstractContainerScreen<GraviticCondenserMenu> {
    private static final ResourceLocation TEXTURE = ResourceLocation.fromNamespaceAndPath(Matterworks.MOD_ID, "textures/gui/gravitic_condenser.png");
    ...
    private static final int UNSTABLE_TANK_X = 119;
    ...
    public GraviticCondenserScreen(GraviticCondenserMenu menu, Inventory playerInventory, Component title) {
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (sideConfig.isShowing()) {
            ...
            renderRawTankTooltip(guiGraphics, mouseX, mouseY);
            renderUnstableTankTooltip(guiGraphics, mouseX, mouseY);
        }
    ...
    private void renderRawTankTooltip(GuiGraphics guiGraphics, int mouseX, int mouseY) {
        int x = this.leftPos + RAW_TANK_X;
    ...
    private void renderUnstableTankTooltip(GuiGraphics guiGraphics, int mouseX, int mouseY) {
        int x = this.leftPos + UNSTABLE_TANK_X;
        int y = this.topPos + TANK_Y;
        if (mouseX >= x && mouseX < x + TANK_WIDTH && mouseY >= y && mouseY < y + TANK_HEIGHT) {
            guiGraphics.renderTooltip(this.font, Component.translatable("tooltip.matterworks.unstable_matter_tank", menu.getUnstableAmount(), menu.getUnstableCapacity()), mouseX, mouseY);
        }
    }
